def main() -> None:
    ...
    COLORE_testo2 = (20, 20, 60)                 # blu navy
    COLORE_testo3 = (200, 60, 60)                # rosso mattone
    COLORE_testo4 = (160, 50, 50)                # rosso mattone scuro
    ...
    def disegna_finestra_impostazioni():
        ...
        # Testo diviso in righe
        righe = [
            "Benvenuto al gioco impostore,",
            "il tuo obiettivo è quello di eliminare tutti",
            "gli impostori presenti tra i partecipanti",
            "prima che rimangano solo 2 giocatori,",
            "uno dei quali è un impostore"
        ]

        # Altezza di una riga (per calcolare la spaziatura)
        altezza_riga = FONT_testo.get_height() + 5  # 5px di margine tra le righe

        # Altezza totale del blocco di testo
        altezza_totale = altezza_riga * len(righe)

        # Punto di partenza Y (per centrare verticalmente il blocco)
        y_start = (ALTEZZA // 2) - (altezza_totale // 2)

        # Disegna ogni riga centrata
        for i, riga in enumerate(righe):
            superficie = FONT_testo.render(riga, True, COLORE_testo)
            rect = superficie.get_rect(center=(LARGHEZZA // 2, y_start + i * altezza_riga))
            schermata.blit(superficie, rect)
    ...
    def disegna_schermata_votazioni():
        ...
        # voti.keys()              prende tutti i nomi del dizionario
        # key=lambda n: voti[n]    è il criterio di ordinamento --> In pratica dice di non ordinare i nomi alfabeticamente, ma in base al loro voto
        # reverse=True             ordine decrescente (dal voto più alto al più basso)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if schermata_attuale == "finestra_principale":
                    if PULSANTE_1.collidepoint(mousePos):
                        schermata_attuale = "inizio_partita"
                    elif PULSANTE_2.collidepoint(mousePos):
                    ...
                    if len(nomi_giocatori) >= 3:
                        if PULSANTE_INIZIA.collidepoint(mousePos):
                    ...
                    if pulsante_conferma_gioco is not None and coperta_rect_gioco is not None:
                        ...
                        if pulsante_conferma_gioco.collidepoint(event.pos):
                            ...
                            if indice_giocatore_corrente >= len(ordine_giocatori):
                                ...
                                indice_giocatore_corrente = 0  # <-- RESET DELL'INDICE
